...
def upload_file():
    if request.method == 'POST':
        ...
        kategori = request.form.get('kategori')  # Formdan kategori alınıyor
        ...
        beyanname = BeyannameKayitlari(
            ...
            kategori=kategori  # Kategori veritabanına kaydediliyor
        )
...
def search():
    ...
    results = [
        (
            ...
            file.kategori  # Kategori sütunu eklendi
...
def download_csv():
    ...
    csv_data.append([
        ...
        'DÖVİZ TUTARI', 'TL TUTARI', 'Gümrük', 'İntaç Tarihi', 'GGB Tarihi', 'Kategori'
    ])
    ...
    for file in files:
        csv_data.append([
            ...
            file.intac_tarihi.strftime('%d.%m.%Y') if file.intac_tarihi else 'Boş',
            file.ggb_tarihi.strftime('%d.%m.%Y') if file.ggb_tarihi else 'Boş',  # GGB tarihi doğru alınmalı
