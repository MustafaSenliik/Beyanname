...
def upload_file():
    if request.method == 'POST':
        ...
        log = Log(
            user_id=current_user.id,  # User ID burada saklanıyor
            action="Dosya Yüklendi",
            details=f"{kodu} kodlu dosya yüklendi. Kullanıcı: {current_user.ad_soyad}"  # ad_soyad bilgisi details kısmına ekleniyor
        )
        db.session.add(log)
        db.session.commit()
...
def delete_file(file_id):
    ...
    if file:
        ...
        log = Log(
            ...
            details=f"{kodu} kodlu dosya silindi. Kullanıcı: {current_user.ad_soyad}"  # ad_soyad bilgisi details kısmına ekleniyor
        )
...
def download_csv():
    ...
    for file in files:
        print(f"Kur değeri: {file.kur}")  # CSV'ye yazmadan önce kur değerini kontrol et
        ...
        csv_data.append([
            ...
            '{:.4f}'.format(file.kur) if file.kur else 'Boş',  # kur değeri burada biçimlendirilmiş
...
def download_filtered_csv():
    # GET parametrelerini alıyoruz
    kategori = request.args.get('kategori', '').strip() or None
    kodu = request.args.get('kodu', '').strip() or None
    intac_start_date = request.args.get('intac_start_date', '').strip() or None
    intac_end_date = request.args.get('intac_end_date', '').strip() or None
    ggb_start_date = request.args.get('ggb_start_date', '').strip() or None
    ggb_end_date = request.args.get('ggb_end_date', '').strip() or None

    # Eğer hiçbir filtre parametresi girilmediyse kullanıcıya hata göster
    if not (kategori or kodu or intac_start_date or intac_end_date or ggb_start_date or ggb_end_date):
        flash('Filtre uygulanmadı! Lütfen en az bir filtre girin.', 'danger')
        return redirect(url_for('search_page'))
    ...
    # Kategori filtresi
    if kategori:
        query = query.filter(BeyannameKayitlari.kategori == kategori)
        print(f"Kategori filtresi uygulandı: {kategori}")

    # Kodu filtresi
    if kodu:
        query = query.filter(BeyannameKayitlari.kodu == kodu)
        print(f"Kodu filtresi uygulandı: {kodu}")
    ...
    if intac_start_date and intac_end_date:
        try:
            ...
            intac_end_date = datetime.strptime(intac_end_date, '%Y-%m-%d')
            query = query.filter(BeyannameKayitlari.intac_tarihi.between(intac_start_date, intac_end_date))
            print(f"İntaç tarihi filtresi uygulandı: {intac_start_date} - {intac_end_date}")
        except ValueError:
            flash('Geçersiz İntaç tarih formatı.', 'danger')
            return redirect(url_for('search_page'))
    ...
    if ggb_start_date and ggb_end_date:
        try:
            ...
            ggb_end_date = datetime.strptime(ggb_end_date, '%Y-%m-%d')
            query = query.filter(BeyannameKayitlari.ggb_tarihi.between(ggb_start_date, ggb_end_date))
            print(f"GGB tarihi filtresi uygulandı: {ggb_start_date} - {ggb_end_date}")
        except ValueError:
            flash('Geçersiz GGB tarih formatı.', 'danger')
            return redirect(url_for('search_page'))

    # Filtrelenmiş dosyaları al
    files = query.all()
    print(f"Filtrelenmiş dosya sayısı: {len(files)}")

    # Eğer sonuç yoksa kullanıcıyı bilgilendir
    if not files:
        flash('Filtreye uygun dosya bulunamadı.', 'warning')
        return redirect(url_for('search_page'))
    ...
    for file in files:
        ...
        csv_data.append([
            ...
            file.miktar if file.miktar else 'Boş',
            file.doviz_cinsi if file.doviz_cinsi else 'Boş',
            "{:.4f}".format(file.kur) if file.kur else 'Boş',  # Kur doğru formatlanmış
            file.doviz_tutari if file.doviz_tutari else 'Boş',
            file.tl_tutari if file.tl_tutari else 'Boş',
            file.gumruk if file.gumruk else 'Boş',
            file.intac_tarihi.strftime('%d.%m.%Y') if file.intac_tarihi else 'Boş',
